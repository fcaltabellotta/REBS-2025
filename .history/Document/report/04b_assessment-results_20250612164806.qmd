...
The estimate of initial recruitment ($lnR_0$) is much higher than the previous assessment (`r round(model_output$parameters[model_output$parameters$Label == "SR_LN(R0)", 3],3)` vs. 6.20). While this is a large increase in the scale of the stock, a value of `r round(model_output$parameters[model_output$parameters$Label == "SR_LN(R0)", 3],3)` is not unusual for shelf and/or slope rockfish species. The estimate of ($lnR_0$) for `r spp` is well within the range of other groundfishes in similar habitat (@fig-lnR0). And given this assessment is for two species, the estimate for ($lnR_0$) is reasonable. There is also a very large variability estimated for this parameter (Coefficient of variation = `r round(model_output$parameters[model_output$parameters$Label == "SR_LN(R0)", 11],3)`/`r round(model_output$parameters[model_output$parameters$Label == "SR_LN(R0)", 3],3)`), thus scale is generally very poorly informed in this model. 
