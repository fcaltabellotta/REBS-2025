...
The coefficient of variation (CV) of length by age are shown in Figure AL_2. This is a measure of the variation in length for a given age class. Sample sizes are highest from the youngest ages up to around 100 years. The amount and range of age samples, along with repeated length samples within an age class, offers hope that these parameters 

Given the amount and range of ages provided in this data set, we are hopeful we can estimate all or most of the growth parameters (Linf, k, t0, and CVs at age) can be estimated internally in the stock assessment. This will require using the ages as conditioned on lengths.

Von Bertalanffy growth parameters are expected to be estimated in the model given the amount of data. Below are the external estimates of the von Bertalanffy parameters by sex and source. Females and males have similar growth curves, though females seem to get to a slightly larger size.
...
```
![Figure AL_2. Coefficient of variation by age for all sources of Rougheye/Blackspotted rockfishes ages. Sample sizes (N) are also indicated by size of the point.](C:/Users/Jason.Cope/Documents/Github/REBS-2025/docs/Pre_assessment/plots/Lt_age_CV.png){width="75%"} 
...
We attempt to estimate natural mortality, as was done in the previous West Coast assessment. Examining the available age data, the oldest 10 individuals ranged from 139 to 165, and were all males. For females, the 10 oldest individuals range from 119-130 years. If those oldest ages were used in the Hame and Cope (2022) longevity estimator, this would be a range of natural mortality values of 0.039 to 0.033 for males, which include the mean of the prior used in the 2013 assessment. For females, it ranges from 121-139 years. This corresponds to natural mortality values of 0.045 to 0.039. All these assume that the sampled population has enough of an age structure still available for sampling, as opposed to having some level of age truncation from the theoretical unfished stable age distribution.  Applying a catch curve analysis (taking the log of the abundance of numbers of samples in available age classes) on the aggregated ages across all age sources by sex, the total mortality (Natural + Fishing mortality= Total mortality) is 0.042 for males and for females, again indicating the natural mortality may be lower than used in the 2013 assessment, but within values considered in other areas (Figure CC_M).
