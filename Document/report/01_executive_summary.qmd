...
Fraction unfished shows more stock dynamics, with slight decline in the 1960s, subsequent increase through the early-1980s, following another decrease over the past forty five years. 
...
`r Spp`  are managed as part of the minor slope rockfish complex, and there were species specific contributions to the OFL catch levels set for the complex. The last ten years total dead catches for `r Spp` against he overfishing limits (OFLs), the acceptable biological catches (ABCs), the annual catch limits (ACLs) are shown in @tbl-es-management. 
...
The model estimated uncertainty around the 2025 spawning output for is 𝜎 = 0.6836465. The uncertainty around the OFL is 𝜎 = 0.6713586. These 𝜎 values underestimate the overall uncertainty as they do not incorporate the model structural uncertainty and do not account for any time-varying dynamics other than recruitment.The estimated uncertainty values are lower than the Category X default 𝜎=XX, so all projections will use the default 𝜎.
