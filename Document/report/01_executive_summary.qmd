...
# had to use GMT015 table (because REBS is part of slope RF) and combined for the executive summary only
...
recent_management_table <- recent_management_table |>
  ...
  select(Year, `OFL (mt).x`, `ABC (mt).x`, `ACL (mt).x`, `Total dead catch (mt)`) |>
  rename(`OFL (mt)` = `OFL (mt).x`,
         `ABC (mt)` = `ABC (mt).x`,
         `ACL (mt)` = `ACL (mt).x`)
