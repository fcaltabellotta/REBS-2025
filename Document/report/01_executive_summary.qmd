...
In the last ten years total dead catches of `r Spp` have been below the annual catch limit. The last ten years total dead catches for `r Spp` against he overfishing limits (OFLs), the acceptable biological catches (ABCs), the annual catch limits (ACLs) are shown in @tbl-es-management. 
...
The model estimated uncertainty around the 2025 spawning output for is 𝜎 = 0.6836465. The uncertainty around the OFL is 𝜎 = 0.6713586. These 𝜎 values underestimate the overall uncertainty as they do not incorporate the model structural uncertainty and do not account for any time-varying dynamics other than recruitment. The estimated uncertainty values are higher than the Category 1 default 𝜎=0.5, so all projections will use the estimated 𝜎.
