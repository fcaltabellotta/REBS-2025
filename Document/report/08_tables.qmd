...
management_raw <- read.csv(here::here("Document", "report", "tables_4_doc", "management-performance.csv"),
                           header = FALSE, stringsAsFactors = FALSE)

header_1 <- as.character(management_raw[1, ])
header_2 <- as.character(management_raw[2, ])

header_1_filled <- fill(tibble(h1 = header_1), h1)$h1

combined_headers <- paste0(header_1_filled, "_", header_2)
combined_headers[1] <- "Year"  # fix "Year" column

management <- management_raw[-c(1,2), ]
colnames(management) <- combined_headers

management <- type_convert(management)

gt_table <- management %>%
  gt::gt(rowname_col = "Year") %>%
  gt::tab_spanner(
    label = "North of 40'10'",
    columns = c(`North of 40.10._OFL`, `North of 40.10._ABC`, `North of 40.10._ACL`)
  ) %>%
  gt::tab_spanner(
    label = "South of 40'10'",
    columns = c(`South of 40.10_OFL`, `South of 40.10_ABC`, `South of 40.10_ACL`)
  ) %>%
  gt::tab_spanner(
    label = "Combined",
