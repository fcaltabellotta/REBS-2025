...
# Create and return the gt table (do not assign or call as_latex manually)
management %>%
  gt(rowname_col = "Year") %>%
  tab_spanner(label = "North of 40'10'", columns = c(north_of_40_ofl, north_of_40_abc, north_of_40_acl)) %>%
  tab_spanner(label = "South of 40'10'", columns = c(south_of_40_ofl, south_of_40_abc, south_of_40_acl)) %>%
  tab_spanner(label = "Combined", columns = c(combined_ofl, combined_abc, combined_acl)) %>%
  tab_options(
    table.font.size = 12,
    latex.u
