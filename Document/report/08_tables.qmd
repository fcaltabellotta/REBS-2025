...
# Read the CSV and skip the two-layer header row
management <- read_csv(
...
# Generate gt table and return it directly
management %>%
  gt(rowname_col = "Year") %>%
  tab_spanner("North of 40'10'", c(north_of_40_ofl, north_of_40_abc, north_of_40_acl)) %>%
  tab_spanner("South of 40'10'", c(south_of_40_ofl, south_of_40_abc, south_of_40_acl)) %>%
  tab_spanner("Combined", c(combined_ofl, combined_abc, combined_acl)) %>%
  tab_options(
    table.font.size = 12,
    latex.use_longtable = TRUE
  )
