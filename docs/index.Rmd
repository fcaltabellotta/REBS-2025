  ...
  label, width = 100, height = 100) 
{
  ...
  if (grepl(",", alt_caption)) {
    stop(cli::format_error(c("\n", x = "{.var alt_caption} cannot contain ','.", i = "Please remove the comma from the following text:","{alt_caption}")))
  }
  if (is.null(alt_caption)) {
    alt_caption <- ""
  }
  if (knitr::is_html_output()) {
    cat(sep = "", "<figure><img src=\"", filein, "\" alt=\"", alt_caption, "\"", sprintf(" style=\"width: %f%%\"", width), "/><figcaption>", caption, "</figcaption></figure>")
  }
  else {
...
add_figure(
  filein = file.path(dir, "blackspotted_bottom_crop.png"), 
  caption = '',
  width = 60,
  height = 60,
  label = 'black')
